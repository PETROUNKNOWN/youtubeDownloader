...
class YouTubeAudioDownloader:
    def __init__(self, root):
        ...
        self.link_entry = ctk.CTkEntry(main_frame, width=500, height=40, placeholder_text="Paste YouTube link...")
        self.link_entry.grid(row=0, column=0, sticky="nsew")

        self.download_button = ctk.CTkButton(main_frame, width=150, height=40, text="Download", command=self.download_audio)
        self.download_button.grid(row=0, column=1, sticky="nsew")

        self.console = ctk.CTkTextbox(main_frame, height=300, wrap="word")
    ...
    def start_download(self, link):
        try:
            ...
            self.log_to_console(f"Error: An unexpected error occurred: {e}")
